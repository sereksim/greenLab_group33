...
RESULTS_PATH = PROJECT_ROOT / "src" / "libraries" / "pytorch_nn_regression"
...
def save_measurements(r2: float, mse: float, rmse: float, mae: float) -> None:
    ...
    results = pd.DataFrame([{
        ...
        "model": "PyTorch NN Regression (CPU)",
        "r2_score": r2,
        "MSE": mse,
        "RMSE": rmse,
        "MAE": mae,
    ...
    csv_path = RESULTS_PATH / "metrics_ptNNR.csv"
    json_path = RESULTS_PATH / "metrics_ptNNR.json"
...
def run_pytorch_nn_regression():
    ...
    print(f"PyTorch NN Regression (CPU)")
