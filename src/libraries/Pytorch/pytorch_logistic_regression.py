...
RESULTS_PATH = PROJECT_ROOT / "src" / "libraries" / "pytorch_logistic_regression"

def save_measurements(accuracy: float) -> None:
    RESULTS_PATH.mkdir(parents=True, exist_ok=True)

    results = pd.DataFrame([{
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        "model": "PyTorch Logistic Regression (CPU)",
        "accuracy": accuracy,
    ...
    csv_path = RESULTS_PATH / "metrics_ptLogR.csv"
    json_path = RESULTS_PATH / "metrics_ptLogR.json"
...
def run_pytorch_logistic():
    ...
    y_binary = (data['median_house_value'] > data['median_house_value'].median()).astype(int)
    X = data.drop(columns=['median_house_value'])

    X_train, X_test, y_train, y_test = train_test_split(
        X, y_binary, test_size=TEST_SIZE, random_state=RANDOM_SEED, stratify=y_binary
    )
    ...
    with torch.no_grad():
        ...
        y_pred = (torch.sigmoid(logits) > 0.5).float().numpy()

    y_test_np = y_test_tensor.numpy()
    acc = accuracy_score(y_test_np, y_pred)
    
    save_measurements(acc)

    print(f"PyTorch Logistic Regression (CPU)")
    print(f"Accuracy: {acc:.4f}")


if __name__ == "__main__":
    run_pytorch_logistic()
