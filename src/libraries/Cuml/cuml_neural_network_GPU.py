...
def save_measurements(accuracy: float, precision: float, recall: float, f1: float) -> None:
    ...
    csv_path = RESULTS_PATH / "metrics_cuml_MLP_GPU.csv"
    json_path = RESULTS_PATH / "metrics_cuml_MLP_GPU.json"
...
def run_cuml_MLP_GPU():
    ...
    if label_col is None:
        raise ValueError("No label column containing 'M'/'B' found.")
